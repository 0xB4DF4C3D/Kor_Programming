...
class MyApp:
    ...
    def __init__(self, parent):
        ...
        self.BottomContainer = Frame(self.myParent)
        self.BottomContainer.pack()
        self.myParent.resizable(False, False)


        self.Input = Text(self.TopContainer, width = 35, height = 20)
        self.Input.insert(END,"Input")
        self.Input.pack(side = LEFT, fill = BOTH)

        self.ButtonContainer = Frame(self.TopContainer)
        self.ButtonContainer.pack(side = LEFT, ipady = 10)

        self.Compile = Button(self.ButtonContainer, text = "Compile", command = self.CompileClick)
        self.Compile.pack(side = TOP)
        self.Decode = Button(self.ButtonContainer, text = "Decode")
        self.Decode.pack(side = BOTTOM)

        self.Decoder = Text(self.TopContainer, width = 35, height = 20)
        self.Decoder.insert(END,"Decoder")
        self.Decoder.pack(side = LEFT, fill = BOTH)

        self.Interpreter = Text(self.BottomContainer, height = 20)
        self.Interpreter.insert(END,"Interpreter")
        self.Interpreter.pack(side = BOTTOM , fill = BOTH, expand = 1)

    def CompileClick(self):
        global director, ttl, ini, mid, fin, ss, c1,c2,c3,c4, s1,s2, cur, cval , sval
