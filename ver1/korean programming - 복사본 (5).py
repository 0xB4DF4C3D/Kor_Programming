...
def move():
    ...
    if not hangul.is_hangul(cur): ini = mid = fin = ''
    if hangul.is_consonant(cur) :ini = hangul.get_initial(cur); fin = hangul.get_final(cur)   #in first, extract initial,vowel,finish from the cur
